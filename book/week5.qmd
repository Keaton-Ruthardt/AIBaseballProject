...
### Post Week 5: MAE findings and results

**Process**:
- Tested all ~100 videos and calculated how far off our predictions were from our model to the actual result.
- For example, if our prediction gave safe with a 80% predicition and the actual result was safe (1). Then our error was 0.2. 
- Tested this for each individual video and then averaged our errors. 

**Results**:
- Our MAE was ~0.32
- This represents that our model is on average 32% off of the exact (1) safe play or the exact (0) out play.

AI Usage:
ChatGPT said:

I used AI (Claude Sonnet) to help refine the wording and organization of this section, including the description of the demo video components, function documentation, and the explanation of the MAE calculation and results. The demo video function, video processing logic, MAE computations, and underlying experimental setup were designed, coded, and executed by me.
...
AI Usage:
I used AI (Claude Sonnet) to help refine the wording, structure, and organization of the project documentation, including example commands, section headings, and usage explanations. All actual documentation content, pipeline design, and example workflows were based on my own project work and implementation.
...
**Model Performance** :
1. Accuracy (0.82)
  The model correctly predicted Safe/Out 82% of the time.

2. Recall (0.78)
       Recall for “Safe” = Of all plays that were actually Safe, the model correctly labeled 78% of them as Safe.

3. Precision (0.83)
     Precision for “Safe” = When the model predicts Safe, it’s correct 83% of the time.

4. F1 Score (0.81)
             F1 balances precision and recall especially important when the cost of errors is not equal.
...
```
AI Usage:
Samuel used AI (GPT 5) to help refine the wording and structure of this performance summary, including the explanations of each metric and the pipeline performance section. All metric calculations, performance results, and interpretation of model behavior were based on his own code, evaluation scripts, and analysis.
