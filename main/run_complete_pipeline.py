...
def run_tracker(video_path, output_csv, output_video=None):
    ...
    print("   Processing frames 300-450 (outfield action)")
    results = tracker.process_video(video_path, output_path=output_video, start_frame=300, end_frame=450)
...
def extract_features(tracker_csv, statcast_csv, video_name):
    ...
    # Get player distance if available (distance between outfielders)
    player_distance = None
    if 'catch_distance_px' in catch_frame and pd.notna(catch_frame['catch_distance_px']):
        # Convert pixel distance to feet
        player_distance = catch_frame['catch_distance_px'] / converter.pixels_per_foot_horizontal
        print(f"   Player distance: {player_distance:.1f} feet")
    ...
    # Combine all features (bearing and launch_direction removed per user request)
    features = {
...
def run_model_prediction(features_df):
    ...
    print("\n[2/4] Initializing and training ensemble model...")
    try:
    ...
    for feature in ['fielder_pos', 'runner_base']:
        if feature in X_predict.columns and feature in predictor.label_encoders:
            ...
            # Handle unseen categories by using 'missing'
            try:
                X_predict[feature] = le.transform(X_predict[feature].fillna('missing'))
            except ValueError:
                # If category not seen during training, use 'missing'
                X_predict[feature] = le.transform(['missing'] * len(X_predict))
